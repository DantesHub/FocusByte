...
let allClothes = topBook.merging(pantsBook, uniquingKeysWith: { (current, _) in current }).merging(shoeBook, uniquingKeysWith: { (current, _) in current }).merging(backpackBook, uniquingKeysWith: { (current, _) in current })
